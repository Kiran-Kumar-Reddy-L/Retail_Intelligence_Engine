...
class RIEDataProcessingPipeline(PreprocessingPipeline):
    ...
    def __init__(self, config: AnyStr):
        ...
        self.preprocessed_output_data_paths = config.get("preprocessed_output_data_paths")
    ...
    def load_and_preprocess_amazon_sale_report_data(self):
        ...
        data = self.load_data(self.raw_input_data_paths["amazon_sales_report"], sep=",", encoding="utf-8")
        
        # Preprocess data
        preprocessed_data = self.preprocess_data(data)

        # Save preprocessed data
        self.write_data(preprocessed_data, self.preprocessed_output_data_paths["amazon_sales_report_preprocessed"])
    ...
    def load_and_preprocess_generic_sale_report_data(self):
        ...
        data = self.load_data(self.raw_input_data_paths["sales_report"], sep=",", encoding="utf-8")
    ...
    def load_and_preprocess_pl_report_data(self):
        ...
        data_march = self.load_data(self.raw_input_data_paths["pl_report_march"], sep=",", encoding="utf-8")

        # Load data for may p&l report
        self.logger.info("Load and Preprocess the profit and loss report data.")
        data_may = self.load_data(self.raw_input_data_paths["pl_report_may"], sep=",", encoding="utf-8")
        ...
        # Save preprocessed data
        self.write_data(preprocessed_data, self.preprocessed_output_data_paths["pl_report_preprocessed"])
