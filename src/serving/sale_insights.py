...
class SaleInsights:
    ...
    def __init__(self, config: AnyStr):
        ...
        Args:
            config (AnyStr): Path to the configuration file.
        """
        with open(config, "rb") as f:
            self.config = tomllib.load(f)

        self.logger = Logger.get_logger(self.__class__.__name__)
        self.drop_columns = self.config.get("data").get("drop_columns", [])
        self.dtype_columns = self.config.get("data").get("dtype_columns", {})
        self.status_mapping = self.config.get("data").get("status_mapping", {})

    def load_data(self, data_in_path: AnyStr, **kwargs) -> pd.DataFrame:
        """Load data from a file.

        Args:
            data_in_path (AnyStr): Path to the input data file.
    ...
    def apply_advanced_preprocessing(
        ...
        for sku in na_amt_skus:
            ...
            if len(mode) > 0:
                filter_df.loc[
                    ((filter_df["sku"] == sku) & (filter_df["amount"].isna())),
                    "amount"
                ] = mode[0]
            else:
                filter_df.loc[
    ...
    def get_revenue_per_day(
        ...
        grouped_df = filter_df.groupby(group_by).agg(
            revenue_per_day = ("total_amount", "sum")
        ).reset_index()
    ...
    def get_top_skus(
        ...
        grouped_df = data.groupby(group_by).agg(
    ...
    def get_avg_selling_price_and_count(
        ...
        grouped_df["average_selling_price"] = grouped_df["average_selling_price"].fillna(0)
