...
import pandas as pd
import numpy as np
from typing import Any, AnyStr, List, Dict, Optional, Callable
...
app_state = {"raw_data": None, "processed_data": None}
...
async def process_data():
    ...
    try:
        ...
        app_state["processed_data"] = insights.derive_total_amount(app_state["processed_data"])
        ...
        if len(app_state["processed_data"]) > 0:
            ...
            raise HTTPException(status_code=400, detail="No data records found after processing")
...
@router.get("/insights/daily-revenue", response_model=List[DailyRevenueResponse], response_model_exclude_defaults=True)
...
@router.get("/insights/top-skus", response_model=List[TopSkusResponse], response_model_exclude_defaults=True)
@validate_data_availability()
@validate_column_value("month")
async def top_skus(
    month: AnyStr = Query(..., description="Month to filter the data"),
...
@router.get("/insights/asp-order-count", response_model=List[ASPOrderCountResponse], response_model_exclude_defaults=True)
@validate_data_availability()
async def asp_order_count(
    filter_by: Optional[AnyStr] = Query(None, description="Filter by SKU or Category")
):
    """
